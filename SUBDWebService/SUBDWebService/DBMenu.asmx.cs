...
namespace SUBDWebService
{
    ...
    public class DBMenu : System.Web.Services.WebService
    {
        ...
        public string getAllFiles()
        {
                ...
                string ans = "";
                string[] filesEntries = Directory.GetFiles(Server.MapPath("Files"));
                string[] fileEntries = new string[0];
                for(int i=0; i<filesEntries.Length;i++)
            {
                //var q = s.Split('\\');
                //fileEntries.Append( q[q.Length - 1]);
                filesEntries[i] = filesEntries[i].Split('\\').Last();
            }
                ans = String.Join("\n", filesEntries);
                return ans;
        ...
        public int SaveAs(string Path)
        {
            ...
            {
                Path = "Files/" + Path;
                using (FileStream fop = File.Create(Server.MapPath(Path) )) { }
        ...
        public int CreateTable(string Name, string[] arrNames, string[] arrTypes)
        {
            ...
            List<string> Names = new List<string>(arrNames);
